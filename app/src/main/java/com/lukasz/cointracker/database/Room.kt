...
interface CoinDao {
    ...
    @Query("select * from databasecoin where name like '%' || :name || '%' or symbol like '%' || :name || '%' order by market_cap_rank desc limit 20")
    fun getSelectedCoins(name: String): LiveData<List<DatabaseCoin>>

    @Query("select id from databasecoin where name like '%' || :name || '%' or symbol like '%' || :name || '%' order by market_cap_rank desc limit 20")
